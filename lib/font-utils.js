  ...
  if (Array.isArray(target.variants)) {
    for (const variant of target.variants) {
      ...
      if (parsed) {
        ...
        variants.set(key, parsed);
      }
    }
  }

  if (!variants.size && Array.isArray(target.fonts)) {
...
export async function buildFamilyQuery(name, weights, { includeAllVariants = false, metadataFetcher } = {}) {
  if (includeAllVariants) {
    const variants = await resolveAllVariants(name, { metadataFetcher });
    const normal = variants.filter((variant) => !variant.italic).map((variant) => variant.weight);
    const italic = variants.filter((variant) => variant.italic).map((variant) => variant.weight);

    const entries = [];
    if (normal.length) {
      const unique = [...new Set(normal)];
      entries.push(...unique.map((weight) => `0,${weight}`));
    }
    if (italic.length) {
      const unique = [...new Set(italic)];
      entries.push(...unique.map((weight) => `1,${weight}`));
    }

    const axis = italic.length ? "ital,wght" : "wght";
