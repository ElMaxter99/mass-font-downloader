...
test("resolveAllVariants parses variants, fonts and axes", async () => {
  const variants = await resolveAllVariants("Roboto", { metadataFetcher });
  ...
  const fallbackVariants = await resolveAllVariants("Fallback", { metadataFetcher });
  assert.deepEqual(fallbackVariants, [
    { weight: 400, italic: false },
    { weight: 400, italic: true },
    { weight: 700, italic: false }
  ]);

  const generatedVariants = await resolveAllVariants("Variable Family", { metadataFetcher });
  assert.deepEqual(generatedVariants, [
